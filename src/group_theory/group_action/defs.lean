...
/-- Note that the `smul_comm_class M α α` typeclass arguments is usually satisfied by `algebra M α`.
-/
@[to_additive]
...
/-- Note that the `is_scalar_tower M α α` typeclass arguments is usually satisfied by `algebra M α`.
-/
lemma smul_mul_assoc [monoid α] (r : M) (x y : α) [is_scalar_tower M α α] :
  (r • x) * y = r • (x * y) :=
smul_assoc r x y

/-- Note that these typeclass arguments are usually satisfied by `algebra M α`. -/
