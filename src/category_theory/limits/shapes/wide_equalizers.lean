...
* `walking_parallel_chunk` is the indexing category used for wide (co)equalizer diagrams
* `parallel_chunk` is a functor from `walking_parallel_chunk` to our category `C`.
* a `trident` is a cone over a parallel chunk.
...
* a `wide_equalizer` is now just a `limit (parallel_chunk f)`
...
@[derive [decidable_eq, inhabited]] inductive walking_parallel_chunk (J : Type v) : Type v
...
instance (J : Type v) : inhabited (walking_parallel_chunk.hom J zero zero) :=
{ default := hom.id _ }
...
instance walking_parallel_chunk.category : small_category (walking_parallel_chunk J) :=
...
-/
def parallel_chunk : walking_parallel_chunk J ⥤ C :=
...
@[simp] lemma parallel_chunk_obj_zero : (parallel_chunk f).obj zero = X := rfl
@[simp] lemma parallel_chunk_obj_one : (parallel_chunk f).obj one = Y := rfl

@[simp] lemma parallel_chunk_map_left {j : J} : (parallel_chunk f).map (line j) = f j := rfl

/-- Every functor indexing a wide (co)equalizer is naturally isomorphic (actually, equal) to a
    `parallel_chunk` -/
@[simps]
def diagram_iso_parallel_chunk (F : walking_parallel_chunk J ⥤ C) :
  F ≅ parallel_chunk (λ j, F.map (line j)) :=
nat_iso.of_components (λ j, eq_to_iso $ by cases j; tidy) $ by tidy

/-- A trident on `f` is just a `cone (parallel_chunk f)`. -/
abbreviation trident := cone (parallel_chunk f)

/-- A cotrident on `f` and `g` is just a `cocone (parallel_chunk f)`. -/
abbreviation cotrident := cocone (parallel_chunk f)

variables {f}

/-- A trident `t` on the parallel chunk `f : J → X ⟶ Y` consists of two morphisms
...
/-- A cotrident `t` on the parallel_pair `f g : X ⟶ Y` consists of two morphisms
...
by rw [←s.w (line j), parallel_chunk_map_left]
...
lemma cotrident.coequalizer_ext [nonempty J] (s : cotrident f) {W : C} {k l : s.X ⟶ W}
  (h : s.π ≫ k = s.π ≫ l) : ∀ (j : walking_parallel_chunk J),
    s.ι.app j ≫ k = s.ι.app j ≫ l
| zero := by rw [←s.app_one (classical.arbitrary J), category.assoc, category.assoc, h]
| one := h

lemma fork.is_limit.hom_ext [nonempty J] {s : trident f} (hs : is_limit s) {W : C} {k l : W ⟶ s.X}
  (h : k ≫ s.ι = l ≫ s.ι) : k = l :=
